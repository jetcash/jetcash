...
static void parse_peer_and_add_to_container(const std::string &str, std::vector<jetcash::NetworkAddress> &container) {
	jetcash::NetworkAddress na{};
...
const static UUID JETCASH_NETWORK = {{0x1A, 0x10, 0xF1, 0x11, 0x11, 0xF0, 0x01, 0x01, 0x1A, 0x11, 0x12, 0xA1, 0x10,
...
Config::Config(common::CommandLine &cmd)
    ...
    , network_id(JETCASH_NETWORK)
    , p2p_bind_port(P2P_DEFAULT_PORT)
    , p2p_external_port(P2P_DEFAULT_PORT)
    , p2p_bind_ip("0.0.0.0")
    , jetcashd_bind_port(RPC_DEFAULT_PORT)
    , jetcashd_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , jetcashd_remote_port(0)
    , jetcashd_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--jetcashd-authorization")) {
		jetcashd_authorization = common::base64::encode(BinaryArray(pa, pa + strlen(pa)));
	}
	if (const char *pa = cmd.get("--jetcashd-bind-address")) {
		if (!common::parse_ip_address_and_port(pa, jetcashd_bind_ip, jetcashd_bind_port))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--jetcashd-remote-address")) {
		std::string addr         = pa;
		const std::string prefix = "https://";
		if (addr.find(prefix) == 0) {
#if !platform_USE_SSL
			throw std::runtime_error(
			    "Using https in --jetcashd-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			jetcashd_remote_port = boost::lexical_cast<uint16_t>(sport);
			jetcashd_remote_ip   = prefix + sip;
		} else {
			const std::string prefix2 = "http://";
			if (addr.find(prefix2) == 0)
				addr = addr.substr(prefix2.size());
			if (!common::parse_ip_address_and_port(addr, jetcashd_remote_ip, jetcashd_remote_port))
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : jetcash::SEED_NODES) {
			NetworkAddress addr;
