...
void ser_members(jetcash::SendProof &v, ISeria &s) {
	const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX = 102;
	std::string addr;
	if (!s.is_input())
...
void ser_members(api::walletd::CreateTransaction::Request &v, ISeria &s) {
	...
	seria_kv("fee_per_coin", v.fee_per_coin, s);
...
void ser_members(api::jetcashd::GetStatus::Request &v, ISeria &s) {
...
void ser_members(api::jetcashd::GetStatus::Response &v, ISeria &s) {
	ser_members(static_cast<api::jetcashd::GetStatus::Request &>(v), s);
	...
	seria_kv("recommended_fee_per_coin", v.recommended_fee_per_coin, s);
...
void ser_members(api::jetcashd::SyncBlocks::Request &v, ISeria &s) {
...
void ser_members(api::jetcashd::SyncBlocks::Response &v, ISeria &s) {
	seria_kv("blocks", v.blocks, s);
	seria_kv("start_height", v.start_height, s);
	seria_kv("status", v.status, s);
}
void ser_members(api::jetcashd::SyncMemPool::Request &v, ISeria &s) {
	if (!s.is_input())
...
void ser_members(api::jetcashd::SyncMemPool::Response &v, ISeria &s) {
...
void ser_members(api::jetcashd::GetRandomOutputs::Request &v, ISeria &s) {
	seria_kv("amounts", v.amounts, s);
	seria_kv("outs_count", v.outs_count, s);
	seria_kv("confirmed_height_or_depth", v.confirmed_height_or_depth, s);
}
void ser_members(api::jetcashd::GetRandomOutputs::Response &v, ISeria &s) { seria_kv("outputs", v.outputs, s); }
void ser_members(api::jetcashd::SendTransaction::Request &v, ISeria &s) {
	seria_kv("binary_transaction", v.binary_transaction, s);
}
void ser_members(api::jetcashd::SendTransaction::Response &v, ISeria &s) { seria_kv("send_result", v.send_result, s); }
...
/*void ser_members(jetcash::api::walletd::GetBlock::Request &v, ISeria &s) {
        seria_kv("hash", v.hash, s);
        seria_kv("height", v.height, s);
}*/
void ser_members(jetcash::api::walletd::GetTransaction::Request &v, ISeria &s) { seria_kv("hash", v.hash, s); }
...
void ser_members(jetcash::api::jetcashd::SubmitBlock::Response &v, ISeria &s) { seria_kv("status", v.status, s); }
...
void ser_members(jetcash::api::jetcashd::BlockHeader &v, ISeria &s) {
	ser_members(static_cast<jetcash::api::BlockHeader &>(v), s);
