...
public:
	...
	bool parse(const BinaryArray &extra) {
		m_fields.clear();
		return jetcash::parse_transaction_extra(extra, m_fields);
	}
	template<typename T>
	...
	bool get_public_key(crypto::PublicKey &pk) const {
		jetcash::TransactionExtraPublicKey extra_pk;
		if (!get(extra_pk)) {
			return false;
		}
...
private:
	std::vector<jetcash::TransactionExtraField>::const_iterator find(const std::type_info &t) const {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const jetcash::TransactionExtraField &f) { return t == f.type(); });
	}
	std::vector<jetcash::TransactionExtraField>::iterator find(const std::type_info &t) {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const jetcash::TransactionExtraField &f) { return t == f.type(); });
	}

	std::vector<jetcash::TransactionExtraField> m_fields;
};
}

namespace seria {
class ISeria;
void ser(jetcash::TransactionExtraMergeMiningTag &v, ISeria &s);
}
