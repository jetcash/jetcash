...
/*struct WalletTransactionDto {
        ...
        WalletTransactionDto(const jetcash::WalletTransaction &wallet) {
                state         = wallet.state;
        ...
        jetcash::WalletTransactionState state;
        uint64_t timestamp;
        uint32_t block_height;
        Hash hash;
...
version(version) {}
        WalletTransferDto(const jetcash::WalletTransfer &tr, uint32_t version)
...
/*void serialize(WalletRecordDto &v, jetcash::ISerializer &s) {
...
std::string read_cipher(common::IInputStream &source, const std::string &name) {
	std::string cipher;
	//	jetcash::BinaryInputStreamSerializer s(source);
...
std::string decrypt(const std::string &cipher, jetcash::WalletSerializerV1::CryptoContext &crypto_ctx) {
	std::string plain;
...
void deserialize_encrypted(Object &obj, const std::string &name,
    jetcash::WalletSerializerV1::CryptoContext &crypto_ctx, common::IInputStream &source) {
