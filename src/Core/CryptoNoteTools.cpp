...
Hash jetcash::get_base_transaction_hash(const BaseTransaction &tx) {
	if (tx.version < 2)
...
void jetcash::decompose_amount(Amount amount, Amount dust_threshold, std::vector<Amount> &decomposed_amounts) {
...
size_t jetcash::get_maximum_tx_size(size_t input_count, size_t output_count, size_t mixin_count) {
...
bool jetcash::get_tx_fee(const Transaction &tx, uint64_t &fee) {
...
uint64_t jetcash::get_tx_fee(const Transaction &tx) {
	uint64_t r = 0;
	if (!get_tx_fee(tx, r))
		return 0;
	return r;
