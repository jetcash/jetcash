...
Node::Node(logging::ILogger &log, const Config &config, BlockChainState &block_chain)
	...
	if (!config.jetcashd_bind_ip.empty() && config.jetcashd_bind_port != 0)
		m_api.reset(new http::Server(config.jetcashd_bind_ip, config.jetcashd_bind_port,
	...
	m_commit_timer.once(DB_COMMIT_PERIOD_JETCASHD);
	advance_long_poll();
}
...
void Node::sync_transactions(P2PClientJetcash *who) {
...
void Node::P2PClientJetcash::on_msg_bytes(size_t, size_t) {  // downloaded. uploaded
	//    node->peers.on_peer_bytes(get_address(), downloaded, uploaded,
	//    node->p2p.get_local_time());
}

CORE_SYNC_DATA
Node::P2PClientJetcash::get_sync_data() const {
...
std::vector<PeerlistEntry> Node::P2PClientJetcash::get_peers_to_share() const {
	auto result =
	    m_node->m_peer_db.get_peerlist_to_p2p(m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
	return result;
}

void Node::P2PClientJetcash::on_first_message_after_handshake() {
...
void Node::P2PClientJetcash::on_msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	NetworkAddress addr;
...
void Node::P2PClientJetcash::on_msg_handshake(COMMAND_HANDSHAKE::response &&req) {
...
void Node::P2PClientJetcash::on_msg_notify_request_chain(NOTIFY_REQUEST_CHAIN::request &&req) {
...
void Node::P2PClientJetcash::on_msg_notify_request_chain(NOTIFY_RESPONSE_CHAIN_ENTRY::request &&req) {
	m_node->m_downloader.on_msg_notify_request_chain(this, req);
}

void Node::P2PClientJetcash::on_msg_notify_request_objects(NOTIFY_REQUEST_GET_OBJECTS::request &&req) {
...
void Node::P2PClientJetcash::on_msg_notify_request_objects(NOTIFY_RESPONSE_GET_OBJECTS::request &&req) {
	m_node->m_downloader.on_msg_notify_request_objects(this, req);
}

void Node::P2PClientJetcash::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PClientJetcash::on_msg_notify_request_tx_pool(NOTIFY_REQUEST_TX_POOL::request &&req) {
...
void Node::P2PClientJetcash::on_msg_timed_sync(COMMAND_TIMED_SYNC::request &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}
void Node::P2PClientJetcash::on_msg_timed_sync(COMMAND_TIMED_SYNC::response &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}

void Node::P2PClientJetcash::on_msg_notify_new_block(NOTIFY_NEW_BLOCK::request &&req) {
...
void Node::P2PClientJetcash::on_msg_notify_new_transactions(NOTIFY_NEW_TRANSACTIONS::request &&req) {
...
#if jetcash_ALLOW_DEBUG_COMMANDS
void Node::P2PClientJetcash::on_msg_network_state(COMMAND_REQUEST_NETWORK_STATE::request &&req) {
...
void Node::P2PClientJetcash::on_msg_stat_info(COMMAND_REQUEST_STAT_INFO::request &&req) {
	if (!m_node->check_trust(req.tr)) {
		disconnect(std::string());
		return;
	}
...
void Node::advance_long_poll() {
	...
	if (m_long_poll_http_clients.empty())
		return;
	api::jetcashd::GetStatus::Response resp = create_status_response3();
	...
	for (auto lit = m_long_poll_http_clients.begin(); lit != m_long_poll_http_clients.end();) {
		const bool method_status = lit->original_json_request.get_method() == api::jetcashd::GetStatus::method() ||
		                           lit->original_json_request.get_method() == api::jetcashd::GetStatus::method2();
		...
		if (method_status) {
			...
			try {
				api::jetcashd::GetBlockTemplate::Request gbt_req;
				lit->original_json_request.load_params(gbt_req);
				api::jetcashd::GetBlockTemplate::Response gbt_res;
...
</svg></td><td>jetcashd &bull; version
)";
...
bool Node::on_api_http_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	if (!m_config.jetcashd_authorization.empty() &&
	    request.r.basic_authorization != m_config.jetcashd_authorization) {
...
const std::unordered_map<std::string, Node::HTTPHandlerFunction> Node::m_http_handlers = {
    ...
    {api::jetcashd::SyncBlocks::bin_method(), bin_method(&Node::on_wallet_sync3)},
    {api::jetcashd::SyncMemPool::bin_method(), bin_method(&Node::on_sync_mempool3)},
...
const std::unordered_map<std::string, Node::JSONRPCHandlerFunction> Node::m_jsonrpc_handlers = {
    {api::jetcashd::GetLastBlockHeader::method(), json_rpc::make_member_method(&Node::on_get_last_block_header)},
    {api::jetcashd::GetBlockHeaderByHash::method(), json_rpc::make_member_method(&Node::on_get_block_header_by_hash)},
    {api::jetcashd::GetBlockHeaderByHeight::method(), json_rpc::make_member_method(&Node::on_get_block_header_by_height)},
    {api::jetcashd::GetBlockTemplate::method(), json_rpc::make_member_method(&Node::on_getblocktemplate)},
    {api::jetcashd::GetCurrencyId::method(), json_rpc::make_member_method(&Node::on_get_currency_id)},
    {api::jetcashd::SubmitBlock::method(), json_rpc::make_member_method(&Node::on_submitblock)},
    {api::jetcashd::GetRandomOutputs::method(), json_rpc::make_member_method(&Node::on_get_random_outputs3)},
    {api::jetcashd::GetStatus::method(), json_rpc::make_member_method(&Node::on_get_status3)},
    {api::jetcashd::GetStatus::method2(), json_rpc::make_member_method(&Node::on_get_status3)},
    {api::jetcashd::SendTransaction::method(), json_rpc::make_member_method(&Node::handle_send_transaction3)},
    {api::jetcashd::CheckSendProof::method(), json_rpc::make_member_method(&Node::handle_check_send_proof3)},
    {api::jetcashd::SyncBlocks::method(), json_rpc::make_member_method(&Node::on_wallet_sync3)},
    {api::jetcashd::SyncMemPool::method(), json_rpc::make_member_method(&Node::on_sync_mempool3)}};

bool Node::on_get_random_outputs3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::jetcashd::GetRandomOutputs::Request &&request, api::jetcashd::GetRandomOutputs::Response &response) {
...
api::jetcashd::GetStatus::Response Node::create_status_response3() const {
	api::jetcashd::GetStatus::Response res;
	...
	res.recommended_fee_per_coin = m_block_chain.get_currency().coin() / m_block_chain.get_currency().minimum_fee;
...
bool Node::on_get_status3(http::Client *who, http::RequestData &&raw_request, json_rpc::Request &&raw_js_request,
    api::jetcashd::GetStatus::Request &&req, api::jetcashd::GetStatus::Response &res) {
...
bool Node::on_wallet_sync3(http::Client *, http::RequestData &&, json_rpc::Request &&json_req,
    api::jetcashd::SyncBlocks::Request &&req, api::jetcashd::SyncBlocks::Response &res) {
	...
	if (req.max_count > api::jetcashd::SyncBlocks::Request::MAX_COUNT) {
...
bool Node::on_sync_mempool3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::jetcashd::SyncMemPool::Request &&req, api::jetcashd::SyncMemPool::Response &res) {
...
bool Node::handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::jetcashd::SendTransaction::Request &&request, api::jetcashd::SendTransaction::Response &response) {
	NOTIFY_NEW_TRANSACTIONS::request msg;
	Transaction tx;
...
bool Node::handle_check_send_proof3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::jetcashd::CheckSendProof::Request &&request, api::jetcashd::CheckSendProof::Response &response) {
	Transaction tx;
	SendProof sp;
	try {
