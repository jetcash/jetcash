...
static const char USAGE[] =
    R"(jetcashd )" jetcash_VERSION_STRING R"(.
...
#endif
    R"(  --jetcashd-authorization=<usr:pass> HTTP authorization for RPC.)";
...
int main(int argc, const char *argv[]) try {
	...
	platform::ExclusiveLock coin_lock(coinFolder, "jetcashd.lock");

	logging::LoggerManager logManager;
	logManager.configure_default(config.get_data_folder("logs"), "jetcashd-");
	...
	std::cout << "jetcashd started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
