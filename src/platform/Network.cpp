...
void TCPSocket::close() {
	if (read_stream) {
		CFread_streamClose(read_stream);
		CFRelease(read_stream);
		read_stream = nullptr;
	}
	if (write_stream) {
		CFwrite_streamClose(write_stream);
...
bool TCPSocket::connect(const std::string &addr, uint16_t port) {
	...
	CFRelease(host);
	host = nullptr;
	//	CFread_streamSetProperty(read_stream, NSStreamSocketSecurityLevelKey, security_dict_ref);
	if (ssl_addr.first) {
		...
		CFread_streamSetProperty(read_stream, kCFStreamPropertySSLSettings, security_dict_ref);
		CFRelease(security_dict_ref);
		security_dict_ref = nullptr;
	}
	CFStreamClientContext my_context = {0, this, nullptr, nullptr, nullptr};
	if (!CFread_streamSetClient(read_stream,
	        kCFStreamEventHasBytesAvailable | kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered,
	        &TCPSocket::read_callback, &my_context)) {
		close();
		return false;
	}
	if (!CFwrite_streamSetClient(write_stream,
	        kCFStreamEventCanAcceptBytes | kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered,
	        &TCPSocket::write_callback, &my_context)) {
		close();
		return false;
	}
	CFread_streamScheduleWithRunLoop(read_stream, CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
	CFwrite_streamScheduleWithRunLoop(write_stream, CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
	CFread_streamOpen(read_stream);
	CFwrite_streamOpen(write_stream);
	return true;
}

size_t TCPSocket::read_some(void *val, size_t count) {
	if (!read_stream || !CFread_streamHasBytesAvailable(read_stream))
		return 0;
	CFIndex bytes_read = CFread_streamRead(read_stream, (unsigned char *)val, count);
...
size_t TCPSocket::write_some(const void *val, size_t count) {
	if (!write_stream || !CFwrite_streamCanAcceptBytes(write_stream))
		return 0;
	CFIndex bytes_written = CFwrite_streamWrite(write_stream, (const unsigned char *)val, count);
...
void TCPSocket::shutdown_both() {
	if (!is_open())
		return;
	CFDataRef da = (CFDataRef)CFwrite_streamCopyProperty(write_stream, kCFStreamPropertySocketNativeHandle);
	if (!da)
		return;
...
void TCPSocket::read_callback(CFread_streamRef stream, CFStreamEventType event, void *my_ptr) {
	...
	case kCFStreamEventErrorOccurred: {
		CFStreamError error = CFread_streamGetError(stream);
...
void TCPSocket::write_callback(CFwrite_streamRef stream, CFStreamEventType event, void *my_ptr) {
	TCPSocket *s = (TCPSocket *)my_ptr;
	switch (event) {
