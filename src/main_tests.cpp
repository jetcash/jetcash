...
static void fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		jetcash::TransactionExtraMergeMiningTag mm_tag;
		mm_tag.depth = 0;
		block.parent_block.base_transaction.extra.clear();
		mm_tag.merkle_root = get_auxiliary_block_header_hash(block);
		if (!jetcash::append_merge_mining_tag_to_extra(block.parent_block.base_transaction.extra, mm_tag))
			throw std::runtime_error("jetcash::append_merge_mining_tag_to_extra failed");
	}
}
...
void test_blockchain(common::CommandLine &cmd) {
	...
	jetcash::BlockChain::DB::delete_db(config.data_folder + "/blockchain");
	...
	if (!currency.parse_account_address_string(
	        "J4pFpnZYwL6Cf1krpKGNH8TSoEMpyPKLVF5U2JdFEDr5ct1FEiFvhey98w3XuL5DpDPhoKHs8Gv2e16Li7tyGHFLM5evK6H", address))
	...
	for (int i = 0; i != 100; ++i) {
		...
		Height height         = 0;
		if (!block_chain.create_mining_block_template(block, address, jetcash::BinaryArray{}, difficulty, height))
